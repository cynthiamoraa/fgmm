...
app.post("/", (req, res) => {
  ...
  if (text === "") {
    ...
    response = `END Please call this number +254790133397 `;
  } else if (text === "1*2") {
    // This is a second level response where the user selected 1 in the first instance

    // This is a terminal request. Note how we start the response with END
    response = `END Please text this number with the relevant details /n i.e location`;
  } else if (text === "2*1") {
         function sendSms() {
           console.log("wwwww");
           const options = {
         ...
         sendSms();
    response = `END You will receive an sms about "What is FGM`;
  }
